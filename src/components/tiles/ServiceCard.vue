<script setup lang="ts">

import ButtonWithArrow from '../common/ButtonWithArrow.vue'
defineProps<{
  title: string;
  price: string;
  period: string;
  showPrice: Boolean;
  description: string;
  features: string[];
  buttonText: string;
  to: { name: string };
}>();
</script>

<template>
  <div class="relative rounded-2xl bg-white p-8 shadow-md transition-all hover:border-secondary text-left">

    <h3 class="text-2xl font-semibold mb-2 text-foreground">{{ title }}</h3>

    <div v-if="showPrice" class="flex items-baseline gap-2 mb-4">
      <span class="text-4xl font-bold text-foreground">${{ price }}</span>
      <span class="text-muted-foreground text-sm">/{{ period }}</span>
    </div>

    <p v-if="description" class="text-muted-foreground mb-6">
      {{ description }}
    </p>

    <ul v-if="features.length" class="list-disc list-inside space-y-2 mb-6 text-foreground">
        <li v-for="feature in features" :key="feature">{{ feature }}</li>
    </ul>

    <ButtonWithArrow
    :text="buttonText"
    :to="to"
    >
    </ButtonWithArrow>
    
  </div>
</template>
